...
export function isDockAvailable(): boolean {
  try {
    ...
    platformContext.logger.error('Error checking dock availability', error, 'dock');
    return false;
  }
}
...
export async function register(config: {
  ...
  try {
    platformContext.logger.info('Registering dock provider', { id: config.id, title: config.title }, 'dock');
    ...
    try {
      ...
      platformContext.logger.info('Initialized theme from platform', { currentTheme }, 'dock');
    } catch (error) {
      platformContext.logger.warn('Could not read platform theme, defaulting to light', error, 'dock');
      currentTheme = 'light';
    }
    ...
    platformContext.logger.info(`[DOCK_ICON] Dock icon URL: "${dockProvider.icon}"`, undefined, 'dock');
    platformContext.logger.info('[DOCK_ICON] Dock configuration before registration', {
      id: dockProvider.id,
      title: dockProvider.title,
      icon: dockProvider.icon,
    ...
    platformContext.logger.info('[DOCK_ICON] Dock registered successfully - OpenFin Dock.register() completed', {
    ...
    platformContext.logger.error('Failed to register dock', error, 'dock');
    return undefined;
  }
}
...
export async function registerFromConfig(
  ...
  platformContext.logger.info('[DOCK_ICON] registerFromConfig icon resolution:', {
...
export async function show(): Promise<void> {
  try {
    await Dock.show();
    platformContext.logger.info('Dock shown', undefined, 'dock');
  } catch (error) {
    platformContext.logger.error('Failed to show dock', error, 'dock');
    throw error;
  }
}
...
export async function deregister(): Promise<void> {
  try {
    if (registration) {
      ...
      platformContext.logger.info('Dock deregistered', undefined, 'dock');
    }
  } catch (error) {
    platformContext.logger.error('Failed to deregister dock', error, 'dock');
    throw error;
  }
}
...
export async function updateConfig(config: {
  ...
  try {
    ...
    platformContext.logger.info('Updating dock configuration', undefined, 'dock');
    ...
    platformContext.logger.info('Dock configuration updated', { buttonCount: buttons.length }, 'dock');
  } catch (error) {
    platformContext.logger.error('Failed to update dock configuration', error, 'dock');
    throw error;
  }
}
...
async function updateAllDockIcons(): Promise<void> {
  try {
    ...
    platformContext.logger.info(`Updating all dock icons for ${currentTheme} theme`, undefined, 'dock');
    ...
    platformContext.logger.info('✅ All dock icons updated successfully', undefined, 'dock');
  } catch (error) {
    platformContext.logger.error('Failed to update dock icons', error, 'dock');
  }
}
...
export function dockGetCustomActions(): CustomActionsMap {
  return {
    ...
    'launch-component': async (payload: CustomActionPayload): Promise<void> => {
      ...
      if (
        ...
        try {
          ...
          platformContext.logger.error('Failed to launch component', error, 'dock');
        }
      }
    },
    ...
    'reload-dock': async (): Promise<void> => {
      try {
        if (!currentConfig) {
          platformContext.logger.error('Dock not registered - cannot reload', undefined, 'dock');
          return;
        }

        platformContext.logger.info('Reload dock action triggered (full deregister/register cycle)', undefined, 'dock');
        ...
        try {
          ...
          const schemes = await platform.Theme.getSelectedScheme();
          currentTheme = (schemes as any) === 'dark' ? 'dark' : 'light';
          platformContext.logger.info('Re-synced theme from platform before reload', { currentTheme }, 'dock');
        } catch (error) {
          platformContext.logger.warn('Could not read platform theme during reload', error, 'dock');
        }

        // Store current config
        if (!currentConfig) {
          platformContext.logger.warn('No current dock configuration to reload', undefined, 'dock');
          return;
        }
        ...
        // Deregister
        await Dock.deregister();
        platformContext.logger.debug('Dock deregistered', undefined, 'dock');
        ...
        platformContext.logger.debug('Dock re-registered', undefined, 'dock');

        // Show the dock
        await Dock.show();
        platformContext.logger.info('Dock reload complete', undefined, 'dock');
      } catch (error) {
        platformContext.logger.error('Failed to reload dock', error, 'dock');
      }
    },
    ...
    'show-dock-devtools': async (): Promise<void> => {
      try {
        platformContext.logger.info('Opening dock developer tools', undefined, 'dock');

        // Strategy 1: Find workspace applications
        const runningApps = await fin.System.getAllApplications();
        platformContext.logger.debug('Running applications', { apps: runningApps.map((a) => a.uuid) }, 'dock');
        ...
        for (const app of runningApps) {
          ...
          if (
            ...
            try {
              platformContext.logger.debug(`Checking workspace app: ${app.uuid}`, undefined, 'dock');
              ...
              for (const window of childWindows) {
                ...
                if (windowName.includes('dock') || windowName.includes('workspace-dock')) {
                  ...
                  platformContext.logger.info(`DevTools opened for dock window: ${window.identity.name}`, undefined, 'dock');
                  return;
                }
              }
              ...
              platformContext.logger.info(`DevTools opened for workspace main window: ${app.uuid}`, undefined, 'dock');
              return;
            } catch (e) {
              platformContext.logger.debug(`Could not open devtools for ${app.uuid}`, e, 'dock');
            }
          }
        }

        // Strategy 2: Check all windows for dock-related names
        platformContext.logger.debug('Trying alternative approach - checking all windows', undefined, 'dock');
        ...
        for (const appInfo of allWindows) {
          ...
          if (mainWindowName.includes('dock')) {
            try {
              ...
              await window.showDeveloperTools();
              platformContext.logger.info(`DevTools opened for main dock window: ${appInfo.mainWindow.name}`, undefined, 'dock');
              return;
            } catch (e) {
              platformContext.logger.debug(`Failed to open devtools for ${appInfo.mainWindow.name}`, e, 'dock');
            }
          }
            ...
            for (const child of appInfo.childWindows) {
              ...
              if (childName.includes('dock')) {
                try {
                  ...
                  await window.showDeveloperTools();
                  platformContext.logger.info(`DevTools opened for child dock window: ${child.name}`, undefined, 'dock');
                  return;
                } catch (e) {
                  platformContext.logger.debug(`Failed to open devtools for ${child.name}`, e, 'dock');
                }
              }
            }
          }
        ...
        platformContext.logger.warn('Could not find dock window, opening devtools for current window', undefined, 'dock');
        const currentWindow = fin.Window.getCurrentSync();
        await currentWindow.showDeveloperTools();
      } catch (error) {
        platformContext.logger.error('Error opening dock developer tools', error, 'dock');
      }
    },
    ...
    'set-theme': async (payload: CustomActionPayload): Promise<void> => {
      platformContext.logger.info('[DOCK] set-theme action triggered', {
      ...
      if (payload.callerType === CustomActionCallerType.CustomDropdownItem) {
        ...
        try {
          platformContext.logger.info(`[DOCK] Setting platform theme to: ${theme}`, undefined, 'dock');
          ...
          import('@openfin/workspace-platform').then(({ getCurrentSync }) => {
            try {
              ...
              platformContext.logger.info(`[DOCK] Platform theme update initiated (fire-and-forget)`, undefined, 'dock');
            } catch (err) {
              platformContext.logger.warn('[DOCK] Platform theme update failed (non-critical)', err, 'dock');
            }
          }).catch((err) => {
            platformContext.logger.warn('[DOCK] Failed to import workspace-platform (non-critical)', err, 'dock');
          });
          ...
          platformContext.logger.info(`[DOCK] Broadcasting IAB event immediately...`, undefined, 'dock');
          ...
          platformContext.logger.info('[DOCK] ✅ IAB theme change event broadcasted successfully', {
            topic: OpenFinCustomEvents.THEME_CHANGE,
            theme
          }, 'dock');

        } catch (error) {
          platformContext.logger.error('[DOCK] Failed to broadcast theme change', error, 'dock');
        }
      } else {
        platformContext.logger.warn('[DOCK] set-theme called with unexpected caller type', {
          callerType: payload.callerType
        }, 'dock');
      }
    },
    ...
    'toggle-theme': async (): Promise<void> => {
      try {
        ...
        platformContext.logger.info('[DOCK] Theme toggle requested', { newTheme }, 'dock');
                ...
                for (const childWindow of window.childWindows) {
                  for (const view of childWindow.views || []) {
                    try {
                      ...
                      platformContext.logger.warn('[DOCK] Could not update view theme', { view: view.name, error: e }, 'dock');
                    }
                  }
                }
              }
            } catch (e) {
              platformContext.logger.warn('[DOCK] Could not update window theme', { window: window.name, error: e }, 'dock');
            }
          });
        ...
        fin.InterApplicationBus.publish(
          ...
          platformContext.logger.warn('[DOCK] Failed to broadcast theme change', err, 'dock');
        });

        // Update platform theme setting (non-blocking)
        platform.Theme.setSelectedScheme(newTheme as any).catch((err) => {
          platformContext.logger.warn('[DOCK] Platform theme update failed', err, 'dock');
        });

        // Update dock icons (non-blocking)
        updateAllDockIcons().catch((err) => {
          platformContext.logger.warn('[DOCK] Failed to update dock icons', err, 'dock');
        });

        platformContext.logger.info('[DOCK] Theme toggle completed', { newTheme }, 'dock');
      } catch (error) {
        platformContext.logger.error('[DOCK] Failed to toggle theme', error, 'dock');
      }
    },
    ...
    'toggle-provider-window': async (): Promise<void> => {
      try {
        ...
        if (isShowing) {
          ...
          platformContext.logger.info('Provider window hidden', undefined, 'dock');
        } else {
          await providerWindow.show();
          await providerWindow.bringToFront();
          platformContext.logger.info('Provider window shown', undefined, 'dock');
        }
      } catch (error) {
        platformContext.logger.error('Failed to toggle provider window', error, 'dock');
      }
    }
  };
}
...
function buildApplicationsButton(items: DockMenuItem[]): DockButton {
  platformContext.logger.info('🔨 Building Applications button', {
  ...
  function convertToDropdownOptions(menuItems: DockMenuItem[], level: number = 0): any[] {
    return menuItems.map((item) => {
      platformContext.logger.debug(`${'  '.repeat(level)}📋 Processing menu item: ${item.caption}`, {
  ...
  platformContext.logger.info('✅ Applications button built successfully', {
