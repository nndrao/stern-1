...
export const setBaseUrl = (url: string): void => {
  ...
  platformContext.logger.info(`Base URL explicitly set to: ${explicitBaseUrl}`, undefined, 'urlHelper');
};
...
export const clearBaseUrl = (): void => {
  ...
  platformContext.logger.info('Base URL cleared, using auto-detection', undefined, 'urlHelper');
};
...
export const migrateToNewBaseUrl = (newBaseUrl: string, oldBaseUrl?: string) => {
  ...
  platformContext.logger.info(`Migrated from ${oldBase} to ${newBaseUrl}`, undefined, 'urlHelper');
};
  ...
  if (typeof window !== 'undefined' && (window as any).fin) {
    try {
      ...
      if (manifest.customSettings?.baseUrl) {
        ...
        platformContext.logger.info('Base URL initialized from manifest', { baseUrl: manifest.customSettings.baseUrl }, 'urlHelper');
      }
    } catch (error) {
