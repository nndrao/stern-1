...
class IABService {
  ...
  subscribe<T = any>(topic: string, handler: IABMessageHandler<T>): () => void {
    ...
    platformContext.logger.debug('IAB subscribed', { topic }, 'IABService');
    ...
    return () => {
      ...
      platformContext.logger.debug('IAB unsubscribed', { topic }, 'IABService');
    };
  }
  ...
  async broadcast<T = any>(topic: string, payload: T, source?: any): Promise<void> {
    ...
    if (this.isOpenFin) {
      try {
        ...
        platformContext.logger.debug('IAB broadcast (OpenFin)', { topic, payload }, 'IABService');
      } catch (error) {
      ...
      platformContext.logger.debug('IAB broadcast (local)', { topic, payload }, 'IABService');
    }
  }
  ...
  async send<T = any>(
    ...
    if (!this.isOpenFin) {
      platformContext.logger.warn('IAB send not available in browser mode', { topic }, 'IABService');
      return;
    }
    ...
    try {
      ...
      platformContext.logger.debug('IAB sent', { targetUuid, targetName, topic }, 'IABService');
    } catch (error) {
    ...
    try {
      ...
      fin.InterApplicationBus.subscribe(
        ...
        (message: IABMessage, identity: any) => {
          platformContext.logger.debug('IAB message received', { topic, message, identity }, 'IABService');
          this.handleMessage(topic, message);
        }
      );

      platformContext.logger.debug('OpenFin IAB subscription setup', { topic }, 'IABService');
    } catch (error) {
